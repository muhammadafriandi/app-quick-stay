...
const AllRooms = () => {
  ...
  const { navigate, room, currency } = useAppContext()
  ...
  const priceRanges = ["0 to 50", "500 to 1000", "1000 to 2000", "2000 to 3000"]
  const sortOptions = ["Price Low to High", "Price   High to Low", "Newest First"]

  // Handle changes for filters and sorting
  const handleFilterChange = (checked, value, type) => {
    selectedFilters((preveFilters) => {
      const updatedFilters = { ...preveFilters }
      if (checked) {
  ...
  // Function to check if a room matches the selected rooms types
      ...
      <div>
        <div className='flex flex-col items-start text-left'>
          ...
          <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>
            Take advantage of our limited-time offers and special package to enchance your stay and create unfogettable memories.
          </p>
        </div>
            ...
            <div className='md:w-1/2 flex flex-col gap-2'>
              ...
              <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities.map((item, index) => (
                  <div key={index} className='flex items-center gap-2px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>
