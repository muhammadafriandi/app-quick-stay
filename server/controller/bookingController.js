...
// POST /api/bookings/check-avaibility
...
export const createBooking = async (req, res) => {
  try {
    ...
    // Before Booking Check Avaibility
    const isAvailable = await checkAvaibility({ checkInDate, checkOutDate, room })
    ...
    // Caculate totalPrice based on nights
...
export const getUserBookings = async (req, res) => {
  try {
    const user = req.user._id
    const bookings = (await Booking.find({ user }).populate("room hotel")).sort({ ceatedAt: -1 })
...
export const getHotelBookings = async (req, res) => {
  try {
    ...
    // Total Revenue
    const totalRevenue = bookings.reduce((acc, booking) => acc + bookings.totalPrice, 0)
